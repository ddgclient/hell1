...
namespace CtvServices
{
    ...
    public static class RadixConversion
    {
        ...
        public static string BinaryToHex(this string data)
        {
        ...
        public static int BinaryToInteger(this string data)
        {
        ...
        /// <summary>
        /// Converts the given string into binary pattern data (drive or expect). Able to process binary, integer and hex representation.
        /// Number format is {Size}'{Radix}{Number}. Anything without Size'Radix is returned as is.
        /// Examples:
        ///     12'hFFF   --> 111111111111
        ///     4'b0101   --> 0101
        ///     5'd7      --> 00111
        ///     LHLHLLL   --> LHLHLLL
        /// .
        ...
        public static string NumberToPatternSymbol(this string data)
        {
            if (string.IsNullOrEmpty(data))
            {
                throw new ArgumentNullException($"In BaseUtilities String.NumberToPaternSymbol(), string is null or empty.", "data");
            }

            // Format examples: 12'hFFF, 4'b0101, 5'd7
            ...
            {
                if (!int.TryParse(dataFields[0], out var size))
                {
                    throw new ArgumentException($"Data=[{data}] using invalid format for pattern symbol conversion. Size=[{dataFields[0]}] is not an integer.", "size");
                }

                var leadingChar = dataFields[1].ToLower()[0];
                var value = dataFields[1].Substring(1);

                if (leadingChar == 'b')
                {
                    if (value.Length > size)
                    {
                        throw new ArgumentException($"Data=[{data} is not matching expected length=[{size}].");
                    }

                    if (value.Length < size)
                    {
                        value = value.ResizeBinary(size);
                    }

                    return value;
                }

                if (leadingChar == 'h' || leadingChar == 'x')
                {
                    return value.HexToBinary(size);
                }

                if (leadingChar == 'd')
                {
                    return value.IntegerToBinary(size);
                }
            }

            throw new ArgumentException($"Data=[{data}] using invalid format for pattern symbol conversion.");
        }
    }
}
